...
uint16_t tmsK1[0x20] = {0x82C0, 0x8380, 0x83C0, 0x8440, 0x84C0, 0x8540, 0x8600,
                        0x8780, 0x8880, 0x8980, 0x8AC0, 0x8C00, 0x8D40, 0x8F00,
                        0x90C0, 0x92C0, 0x9900, 0xA140, 0xAB80, 0xB840, 0xC740,
                        0xD8C0, 0xEBC0, 0x0000, 0x1440, 0x2740, 0x38C0, 0x47C0,
                        0x5480, 0x5EC0, 0x6700, 0x6D40};
uint16_t tmsK2[0x20] = {0xAE00, 0xB480, 0xBB80, 0xC340, 0xCB80, 0xD440, 0xDDC0,
                        0xE780, 0xF180, 0xFBC0, 0x0600, 0x1040, 0x1A40, 0x2400,
...
/** update_8khz is the worker function to update the audio.
 *
 * Place it in a timer interrupt.  It will return the next 8-bit value to send
 * to the audio system.
 *
 * @return 8-bit value for the audio system.
 */
uint8_t Talkie::update_8khz(void) {
  static uint8_t nextPwm = 128;
  static uint8_t periodCounter = 0;
  static int16_t x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  int16_t u0, u1, u2, u3, u4, u5, u6, u7, u8, u9, u10;
  uint8_t returnValue = nextPwm;
